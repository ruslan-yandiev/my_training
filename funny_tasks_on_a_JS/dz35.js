...
function getNumber(n) {
    ...
    function f(newArr) {
        ...
        for (let i = 0; i < newArr.length; i++) if (i % 2 === 1) arr.push(newArr[i]);
...
function getNumber2(n) {
    function f(newArr) {
        ...
        return f(newArr.filter((elem, index) => index % 2 > 0 ? elem : false).reverse());
    }

    return f(Array.from({length: n}, (_, i) => i + 1)); // передадим в аргументы сгенерированный заполненный массив от [1 до n включительно]
}
...
function getPINs(observed) {
    ...
    if (observed.length === 1) return [observed, ...obj[observed]];

    let arr = [], arr2 = [];

    for (let i = 0; i < observed.length; i++) arr.push([observed[i], ...obj[observed[i]]]);

    function build(numArr) {
        for (let i = 0; i < numArr.length; i++) {
            for (let j = 0; j < arr[1].length; j++) {
                arr2.push(numArr[i] + arr[1][j]);
            }
        }
