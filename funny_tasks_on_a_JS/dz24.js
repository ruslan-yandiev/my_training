...
function realizeDistance(distance, stopAfter) {
    if (distance < stopAfter) return `Вы проехали ${distance} и доехали до точки.`;
    ...
    function steps(d, s) {
        ...
        console.log(`Остановка №${num}. Вы проехали ${sum} метров.`);

        if (sum + s < distance) steps(d - sum, s);

        return `Вы проехали еще ${d - sum} метров и доехали до точки.`;
    }
...
// 2 Используя глобальный объект window Разобраться почему не верно работает и как оно работает.
...
const getRandomString2 = genRandomPostfix2('_prefix_', 4);
