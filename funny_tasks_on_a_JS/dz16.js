...
function calc(arr) {
    ...
    for (let i = 0; i < index; i++) profit = max - arr[i] + profit;

    return arr.length > 1 ? profit + calc(arr.slice(index + 1)) : profit;
}
...
console.log(calc([1, 2, 10, 2, 4])); // 19
console.log(calc([1, 2, 10, 2, 4, 6, 2, 4, 6])); // 29
console.log(calc([1, 8, 5, 8, 8, 7])); // 10 (покупка 1, sell 8, buy 5 sell 8 = 10)
