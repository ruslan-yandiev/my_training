/*
!Библиотеки дополняющие React:
  
*/


/*
! UI Компонент - это блок нашего пользовательского интерфейса с каким то внешним видом, с какимто поведением который в дальнейшем можно переиспользовать в разных частях нашего приложения. Хороший UI компонент должен быть маленьким и независимым от всего остального приложения ,чтобы его легко можно было обновлять и тестировать
...
*/
/*
Все что начинается с большой буквы в <MyComponent/> Риакт будет интерпритировать как наш кастомный компонент, а все что с маленькой <my.../> как HTML элемент-тег

Блок JSX фрагмент вернет после его парсинга Риакт элемент (ну лили тонее будет вызвна функци React.createElement(..., ..., ...);)
Рдин файл, один компонент
Когда мы в пропс передаем свойство, но не передоем его значение то по умолчанию (дефолтно) значение будет булевое true;
------------------------------------------------------------------------------------------------------
в JSX когда имена свойств компонента совподают с именами свойств объекта то можно просто их передать как пропс посредствам спред оператора {...obj}
то есть, есди мы хотим передать все свойства объекта в качестве свойств внутри компонента вместо того, чтобы перечислять их по одному передавая в пропс, можно просто развернуть их через спред оператор.
Пример:
...
---------------------------------------------------------------------------------------------------
Каждый раз когда Риакт рендерит приложение он пытается определить какие именно элементы изменились в дом дереве и обновить только эти части страницы (реконсулейшен алгоритм) И уникальные ключи в списке(коллекции) поможет Риакту обновить только то, что нужно сравнивая елементы из виртуального ДОМ дерева с Реальным ДОМ деревом (грубая аналогия)
Если же не проставим сами ключи, то Риакт их проставит сам по нумерации от 1 .... Это плохо, так как сравнивать он бует последовательно от первого и дальше, и если список изменится не в конце колекции а в начале то возникнет проблема и будут перерированы многие ноды
Вот почему и нет смысла использовать индекс в качестве ключа, так как Риакт и так делает цифровую последовательность если не задать в ручную ключь. Такие списки имеют право на жизнь, но только если список последователен, не меняет свой порядок и новые эленменты добавляются только в конец списка.
...
===============================================================================================================
Изменение состояния это асинхронный процесс!!!! Асинхронность для изменения состояния нужна для React(НАКАПЛИВАЕТ ИЗМЕНЕНИЯ И ПРИМЕНЯЕТ РАЗОМ, ЧТОБЫ ИЗБЕЖАТЬ ЛИШНИХ И ПОВТОРЫНЫХ МАНИПУЛЯЦИЙ С DOM)
...
/*
Бесконечную подгрузку поможет реализовать Intersection Observer API - Intersection Observer API позволяет веб-приложениям асинхронно следить за изменением пересечения элемента с его родителем или областью видимости документа viewport.
...
/*
В React у функций (если писать компоненты на функциях) компонета нет внутреннего состояни, по этому и приходится использовать хук useState

							КЛАССОВЫЕ КОМПОНЕНТЫ
Классовые компоненты используются тогда когда нам необходимо, чтобы именно у компонента сразу было какое то состояни
Способы привязки this
1. Bind в элементе — <elem onClick={ this.elemOnClick.bind(this) } … />
2. Обработчик в конструкторе компонента-класса — constructor() { super(); this.elemOnClick = () => { console.log(`Done: ${this.props.label}`); }; }
3. Поля классов (Class fields) — elemOnClick = () => { console.log(`Done: ${this.props.label}`); };
